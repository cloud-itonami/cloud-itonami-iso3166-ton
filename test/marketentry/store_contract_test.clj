...
(defn- exercise [s]
  (store/commit-record! s {:effect :engagement/upsert
                           :value {:id "eng-x" :operator "X Ltd" :jurisdiction "TON"
                                   :base-fee 100 :monthly-rate 10 :monitoring-months 1
                                   :claimed-fee 110.0
                                   :seeks-firc? true :business-sector :food-beverage-tobacco-retail
                                   :tongan-equity-pct 0.0 :paid-up-capital 250000
  ...
  (store/commit-record! s {:effect :assessment/set
                           :path ["eng-x"]
                           :payload {:jurisdiction "TON" :checklist ["a"] :spec-basis "x"}})
...
(deftest mem-and-datomic-parity
  (let [mem (store/seed-db)
    ...
    (is (= (:tongan-equity-pct (:engagement m)) (:tongan-equity-pct (:engagement d))))
