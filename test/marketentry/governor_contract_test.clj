  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Nuku'alofa Trading Ltd"}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Nuku'alofa Trading Ltd" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest foreign-investment-ineligible-is-held-and-unoverridable
  (testing "declared :seeks-firc? true for a Restricted-List activity whose own equity/capital conditions the engagement's own declared facts do not satisfy -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:foreign-investment-ineligible} (-> (store/ledger db) last :basis)))
