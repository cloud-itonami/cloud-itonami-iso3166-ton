(ns marketentry.store
  "SSoT for the Kingdom of Tonga (TON) market-entry compliance actor,
...
(defn demo-data
  ...
  `:seeks-firc?` / `:business-sector` / `:tongan-equity-pct` /
  `:paid-up-capital` are ground truth for the
  `foreign-investment-violation?` flagship check (Foreign Investment
  Act 2020 s.5; Foreign Investment Regulations 2021 Schedules 1-2);
  `:requires-tin-record?` / `:tin-record-verified?` are ground truth
  for the conditional TIN-record check."
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Nuku'alofa Trading Ltd" :portal "gov.to/procurement"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :seeks-firc? false :business-sector :general-wholesale-and-distribution
             :tongan-equity-pct 1.0 :paid-up-capital nil
             :requires-tin-record? true :tin-record-verified? true
             :drafted? false :submitted? false
             :jurisdiction "TON" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "gov.to/procurement"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :seeks-firc? false :business-sector :general-wholesale-and-distribution
             :tongan-equity-pct 1.0 :paid-up-capital nil
    ...
    "eng-3" {:id "eng-3" :operator "Vava'u Fabrication Co" :portal "gov.to/procurement"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "TON" :status :intake}
    "eng-4" {:id "eng-4" :operator "Ha'apai Agri-Export Ltd" :portal "gov.to/procurement"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             ...
             :drafted? false :submitted? false
             :jurisdiction "TON" :status :intake}
    "eng-5" {:id "eng-5" :operator "'Eua Supplies Cooperative" :portal "gov.to/procurement"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :seeks-firc? false :business-sector :general-wholesale-and-distribution
             :tongan-equity-pct 1.0 :paid-up-capital nil
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               seeks-firc? business-sector tongan-equity-pct paid-up-capital
  ...
  (cond-> {:engagement/id id}
    ...
    (some? seeks-firc?)                   (assoc :engagement/seeks-firc? seeks-firc?)
    (some? business-sector)               (assoc :engagement/business-sector business-sector)
    (some? tongan-equity-pct)             (assoc :engagement/tongan-equity-pct tongan-equity-pct)
    (some? paid-up-capital)               (assoc :engagement/paid-up-capital paid-up-capital)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/seeks-firc? :engagement/business-sector :engagement/tongan-equity-pct :engagement/paid-up-capital
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :seeks-firc? (boolean (:engagement/seeks-firc? m))
     :business-sector (:engagement/business-sector m)
     :tongan-equity-pct (:engagement/tongan-equity-pct m)
     :paid-up-capital (:engagement/paid-up-capital m)
